...
public class AndroidKeyStoreRSAUtils {
    ...
    public static final byte[] DEFAULT_SPLIT = "#NeWolfNeWolf#".getBytes();    // 当要加密的内容超过bufferSize，则采用partSplit进行分块加密
    public static final int DEFAULT_BUFFERSIZE = (DEFAULT_KEY_SIZE / 8) - 11;// 当前秘钥支持加密的最大字节数
    public static final String TAG = AndroidKeyStoreRSAUtils.class.getSimpleName();
    /**
    ...
    public static void init(@NonNull Context context, @NonNull String alias, boolean logEnable) {
        ...
        if (!isHaveKeyStore()){
            ...
            try {
                mKeyPair = generateRSAKeyPair(context.getApplicationContext());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }else{
